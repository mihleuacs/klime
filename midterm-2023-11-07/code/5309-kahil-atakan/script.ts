...
const generateMovieRow =(movie: Movie) => {
    ...
    if (movie.cast.length > 5) {
        ...
        const formattedCast = [...movie.cast].sort((a, b) => a.actor.localeCompare(b.actor)).map((castMember) => `${castMember.actor} as ${castMember.character}*`); castCell.innerHTML = formattedCast.join(""); } row.appendChild(castCell);
